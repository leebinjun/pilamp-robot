...
import Pose.common
from Pose.common import CocoPairsNetwork, CocoPairs, CocoPart
...
class FluidPoseEstimator:
    def __init__(self, graph_path, target_size=(320, 240)):
        ...
        [self.inference_program, self.feed_target_names, self.fetch_targets] = \
            fluid.io.load_inference_model(dirname=graph_path, executor=self.exe)
    ...
    def inference(self, npimg, scales=[None]):
        ...
        npimg = np.array([np.transpose(npimg, (2, 0, 1))]).astype(np.float32)
        output = self.exe.run(self.inference_program,
                      feed={self.feed_target_names[0]: npimg},
